        ...
        currentItems.length === 0 ? "Sorry B*tch there's nothing about what are u looking for :))" : currentItems.filter(burger => {return burger.price > parseInt(price, 10) && burger.price < parseFloat(price2, 10)}).map((slide, index) => {
          return (
            <div className={s.sliderCard1} key={slide.id} onClick={()=>slide.clickEvent()}>
                        ...
                        <p className={s.sliderCardDescription}>{slide.price} <br /> <NavLink to={`/ice-cream/${slide.id}`}>Buy Now!</NavLink></p>
        ...
        currentItems.length === 0 ? "Sorry B*tch there's nothing about what are u looking for :))" : currentItems.filter(burger => {return burger.price > parseInt(price, 10) && burger.price < parseFloat(price2, 10)}).map((slide, index) => {
          return (
            <div className={s.sliderCard2} key={slide.id} onClick={()=>slide.clickEvent()}>
                        ...
                        <p className={s.sliderCardDescription2}>{slide.price} <br /> <NavLink to={`/ice-cream/${slide.id}`}>Buy Now!</NavLink></p>
...
function Section66(props) {
  ...
  function getPosts() {
    Axios(`/ice-cream?name_like=${search}`).then((res) => setPosts(res.data))
  }
  ...
  return (
    <div className="container w-100">
      <div>
        ...
        <h5 className="fw-bold">Ice-cream</h5>

        <div className={s.burgers}>
              <div >
            <img src="img/ice-cream.jpg" width={200} alt="" />
          </div>
            <div>
...
export default connect((state) => state.iceCreamReducer, actions)(Section66);
