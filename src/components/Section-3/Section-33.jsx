        ...
        currentItems.length === 0 ? "Sorry B*tch there's nothing about what are u looking for :))" : currentItems.filter(burger => {return burger.price > parseInt(price, 10) && burger.price < parseFloat(price2, 10)}).map((slide, index) => {
          return (
            <div className={s.sliderCard1} key={slide.id} onClick={()=>slide.clickEvent()}>
                        ...
                        <p className={s.sliderCardDescription}>{slide.price} <br /> <NavLink to={`/burgers/${slide.id}`}>Buy Now!</NavLink></p>
        ...
        currentItems.length === 0 ? "Sorry B*tch there's nothing about what are u looking for :))" : currentItems.filter(burger => {return burger.price > parseInt(price, 10) && burger.price < parseFloat(price2, 10)}).map((slide, index) => {
          return (
            <div className={s.sliderCard2} key={slide.id} onClick={()=>slide.clickEvent()}>
                        ...
                        <p className={s.sliderCardDescription2}>{slide.price} <br /> <NavLink to={`/burgers/${slide.id}`}>Buy Now!</NavLink></p>
...
function Section33(props) {
  ...
  function getPosts() {
    Axios(`/burgers?name_like=${search}`).then((res) => setPosts(res.data))
  }
  ...
  const Values = () => {
    return <select className="custom-select custom-select-lg w-50" value={value} onChange={(e) => setValue(e.target.value)}>
    {
    ...
    <div className="container w-100">
      <div>
        ...
        <div className={s.burgers}>
              <div >
            <img src="img/burger.jpg" width={200} alt="" />
          </div>
            <div>
...
export default connect((state) => state.postReducer, actions)(Section33);
